...
# ── HHI labels on top 5 highest income zip codes ─────────────────────────────
gdf_proj  = gdf_merged.to_crs(epsg=3857)
top5_zips = zip_agg.nlargest(5, "zcta_hhi")["zip_code"].tolist()
...
for _, row in gdf_top5.iterrows():
    ...
    folium.Marker(
        ...
        icon     = folium.DivIcon(
            html = f"""<div style="
                font-size:10px;font-weight:bold;color:#08306B;
                background:rgba(255,255,255,0.85);
                padding:2px 5px;border-radius:3px;
                white-space:nowrap">
                {row['zip_code']}<br>${row['zcta_hhi']/1000:.0f}K HHI
            </div>""",
            icon_size   = (80, 28),
            icon_anchor = (40, 14)
        )
...
<div style="
    ...
    width:220px;height:260px;border:2px solid #333;
    ...
    <div style="
        ...
        padding:4px 8px;z-index:10;letter-spacing:0.5px;">
