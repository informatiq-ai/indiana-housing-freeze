...
Methodology:
  ...
  Price segmentation    — k-means (k=4) on raw sale_price (no log transform); top two
                          tiers collapsed into luxury; natural breaks at ~$388K and ~$1.04M
...
# Confirmed breakpoints (2021–2025 data): $388K (entry/mid_luxury),
# $1.04M (mid_luxury/luxury). Top two clusters (luxury + ultra-luxury)
# collapsed due to thin ultra-luxury sample (n≈48).
...
if abs(breaks[0] - EXPECTED_ENTRY_BREAK) > BREAK_TOLERANCE:
    raise ValueError(
        f"entry/mid_luxury break ${breaks[0]:,.0f} deviates more than "
...
if abs(breaks[1] - EXPECTED_LUXURY_BREAK) > BREAK_TOLERANCE:
    raise ValueError(
        f"mid_luxury/luxury break ${breaks[1]:,.0f} deviates more than "
...
if n_na_segment:
    ...
    fallback_labels = np.where(
        ...
        np.where(fallback_prices <= breaks[1], "mid_luxury", "luxury"),
    )
